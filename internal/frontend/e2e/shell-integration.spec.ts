...
test("未装时显示 banner，一键安装写入 ~/.zshrc（含 marker 与 ccs）", async ({
  page,
  isolatedServer,
}) => {
  ...
  await expect(page.getByText(/Shell integration not yet installed/)).toBeVisible();
  const installBtn = page.getByTestId("shell-install-button");
...
test("安装后刷新页面不再显示 banner", async ({ page, isolatedServer }) => {
  ...
  const res = await page.request.post(
    `${isolatedServer.baseURL}/api/v1/shell-integration/install`,
    {
      data: { shell: "zsh" },
    },
  );
  ...
  await page.goto(isolatedServer.baseURL);
  // 已装：一键安装按钮不应出现。
  await expect(page.getByTestId("shell-install-button")).toHaveCount(0);
});
