...
> 背景见 [工程细节 §6](./engineering-decisions.md#6-claude-的-settingsjson-env-优先级高于-shell已用-claude_config_dir-解决)；数据模型见 [架构 §3](./architecture.md#3-数据模型元信息索引-profile-真值--偏好三层)。
...
> 团队此前真机实测见 [工程细节 §6](./engineering-decisions.md#6-claude-的-settingsjson-env-优先级高于-shell已用-claude_config_dir-解决)。以下两点决定 Mode B 共享边界，**务必先验证**：
...
2. **设 `CLAUDE_CONFIG_DIR` 后，claude 是否完全不读全局 `~/.claude/settings.json`？**
   - 完全不读 → 印证必须把全局非 env 配置合并进 profile settings.json（§4.1）。
   - 会合并 → 可简化。
3. （次要）`sharedEntries` 白名单与目标 claude 版本的实际状态目录一致。
