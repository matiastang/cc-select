...
## 1. 参考事实：现有全局切换工具的技术栈（以 cc-switch 为例）

为了理解同领域工具的实现路径，下面列出 [cc-switch](https://github.com/farion1231/cc-switch) 的真实技术栈作为参考：
...
即：cc-switch 是一个 **Rust（Tauri）+ React** 的轻量桌面 App，通过改写 `~/.claude/settings.json` 实现全局切换。

> 注：cc-switch 是桌面 App + 全局切换路线，与本项目**选定 Web 路线 + shell 级隔离不同**。其前端实现（React + 表单 UI）可作为 Web 配置页 UI 的参考，但 GUI 形态与切换语义本身不照搬。
...
| **Rust** | 追求极致体积/启动，或想走桌面 App（Tauri）同栈 | CLI 开发最繁琐，性能优势在此用不上多少 |
...
| 桌面 App（Tauri+React） | 🔁 备选 | 体验最好、可常驻托盘 | 需打包，体积较大 |
...
| SQLite | 🔁 备选 | 结构化、多表 | 多进程锁竞争，过度设计 |
| 多文件拆分 | 🔁 备选 | 可单独分享某 provider | 全局视图需聚合 |

> 切换机制改用 `CLAUDE_CONFIG_DIR`（见 [architecture §3.0](./architecture.md#30-切换机制claude_config_dir关键)、[engineering §6](./engineering-decisions.md#6-为何改用-claude_config_dir)）。profile settings.json 的 env 字段是 claude 实际读取的配置来源，因此无论 token 是明文还是 keychain 占位，最终写入 profile 时都必须能被 claude 解析为真值。

---

## 5. 跨平台与 shell（Q5/Q6）

- **OS**：macOS / Linux / Windows 三平台。Web GUI 与 JSON 跨平台天然；shell 集成与 Windows 环境变量机制需注意（见 [architecture §3.1](./architecture.md#31-跨平台约束满足-q6macoslinuxwindows)）。
...
- [x] i18n：en / zh（CLI + Web 共享 `prefs.json` 语言偏好）

> Go 下的具体依赖（`cobra`、`zalando/go-keyring`、标准库 `net/http` + `embed`、`react-i18next` 等）已在实现期落地，详见 `go.mod` 与各 `internal/` 包文档。
